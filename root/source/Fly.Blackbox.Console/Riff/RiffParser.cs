...
namespace Fly.Blackbox.Console.Riff
{
    public class RiffParser
    {
        ...
        Dictionary<string, long> _Bookmarks = new Dictionary<string, long>();
        ...
        Dictionary<int, List<_AviIndex>> StreamIndex = new Dictionary<int, List<_AviIndex>>();

        public RiffParser()
        {
        }

        public void AddBookmark(string name, int delta = 0)
        {
            _Bookmarks.Add(name, m_stream.Position + delta);
        }
        ...
        public IEnumerable<KeyValuePair<string, long>> Bookmarks
        {
            get
            {
                return _Bookmarks;
        ...
        public IEnumerable<_AviIndex> GetStream(int pos)
        {
            return StreamIndex[pos];
        }
        ...
        public unsafe void OpenFile(string filename)
        {
            ...
            try
            {
                ...
                m_stream = new FileStream(m_filename, FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite);
                int FourCC;
                int datasize;
                int fileType;
        ...
        /// <returns></returns>
        public bool ReadElement(Action<RiffParser, int, int, int> chunk, Action<RiffParser, int, int> list)
        {
            // Are we done?
        ...
        /// </summary>
        /// <param name="data">Output int</param>
        public unsafe void ReadOneInt(out int data)
        {
            try
            {
        ...
        public int ReadOneInt()
        {
            try
            {
                ...
                return BitConverter.ToInt32(m_fourBytes, 0);
            }
            catch (Exception ex)
            {
                throw new RiffParserException("Problem accessing RIFF file " + FileName, ex);
        ...
        public long ReadTwoInt()
        {
            try
            {
                ...
                return BitConverter.ToInt64(m_eightBytes, 0);
            }
            catch (Exception ex)
            {
                throw new RiffParserException("Problem accessing RIFF file " + FileName, ex);
        ...
        public byte ReadOneByte()
        {
            try
            {
                int readsize = m_stream.Read(m_fourBytes, 0, 1);
        ...
        public void AddIndex(string identifier, int flags, int offset, int length)
        {
            int stream = int.Parse(identifier.Substring(0, 2));

            if (StreamIndex.ContainsKey(stream) == false)
            {
                var list = new List<_AviIndex>();
                StreamIndex.Add(stream, list);
            }

            StreamIndex[stream].Add(new _AviIndex()
            {
